...
CREATE TABLE `ai_box_device` (
  ...
  `is_expired` tinyint(4) NOT NULL DEFAULT '0' COMMENT '是否到期',
  PRIMARY KEY (`id`),
  UNIQUE KEY `dev_uid` (`dev_uid`),
  UNIQUE KEY `uk_merchant_custom_code` (`merchant_id`,`custom_code`),
  UNIQUE KEY `ux_dev_code` (`dev_code`)
) ENGINE=InnoDB AUTO_INCREMENT=2378 DEFAULT CHARSET=utf8mb4 COMMENT='云盒子设备信息表';
