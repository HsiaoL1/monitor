...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (path === "/" || path === "/resource-chart") {
      ...
      path === "/account-sync-log" ||
      path === "/account-restart"
    ) {
      setOpenKeys(["account"]);
      ...
      path === "/browser-servers" ||
      path === "/browser-server-monitor"
    ) {
      setOpenKeys(["browser-farm"]);
    } else if (
      path === "/device-overview" ||
      path === "/device-version-monitor"
    ) {
      setOpenKeys(["device"]);
    }
  }, [location.pathname]);
        ...
        <Layout style={{ padding: "24px" }}>
          ...
          >
            <Routes>
              ...
              <Route
                path="/browser-servers"
                element={<BrowserServerManager />}
              />
              <Route
                path="/browser-server-monitor"
                element={<BrowserServerMonitor />}
              />
              <Route
                path="/browser-account-monitor"
                element={<BrowserAccountMonitor />}
              />
            </Routes>
          </Content>
        </Layout>
