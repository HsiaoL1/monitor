...
const ResourceChart: React.FC = () => {
  const [historyData, setHistoryData] =
    useState<ResourceHistoryResponse | null>(null);
  ...
  // 增量更新图表数据
  const updateChartsIncrementally = useCallback(
    (newData: ResourceHistoryResponse) => {
      ...
      runningServices.forEach((service) => {
        service.dataPoints.forEach((point) => {
          if (
            ...
            point.timestampFormatted > lastUpdateTime.current
          ) {
      ...
      runningServices.forEach((service) => {
        ...
        allNewTimePoints.forEach((timePoint) => {
          const dataPoint = service.dataPoints.find(
            (p) => p.timestampFormatted === timePoint,
          );
          if (dataPoint) {
      ...
      try {
        ...
        runningServices.forEach((service, seriesIndex) => {
          ...
          if (
            serviceData &&
            serviceData.cpu.length > 0 &&
            seriesIndex < currentCpuSeries.length
          ) {
            // 构建新的数据点数组
            const newDataPoints = serviceData.times
              .map((time, index) => [time, serviceData.cpu[index]])
        ...
        runningServices.forEach((service, seriesIndex) => {
          ...
          if (
            serviceData &&
            serviceData.memory.length > 0 &&
            seriesIndex < currentMemorySeries.length
          ) {
            // 构建新的数据点数组
            const newDataPoints = serviceData.times
              .map((time, index) => [time, serviceData.memory[index]])
              .filter((point) => point[1] !== null);
        ...
        console.warn(
          "Incremental chart update failed, falling back to full update:",
          error,
        );
        // 如果增量更新失败，回退到完全更新
  ...
  const fetchHistoryData = useCallback(
    async (isFullRefresh: boolean = false) => {
  ...
  const runningServicesCount = historyData
    ? Object.values(historyData.services).filter((s) => s.status === "running")
        .length
    : 0;
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <Button
            icon={
              isAutoRefresh ? <PauseCircleOutlined /> : <PlayCircleOutlined />
            }
          ...
          {isAutoRefresh && (
            <span style={{ color: "#666" }}>自动刷新间隔: 10秒</span>
          )}
        </div>
      </div>
