...
const AccountMonitor: React.FC = () => {
  ...
  const [selectedRecord, setSelectedRecord] =
    useState<AccountStatusMismatch | null>(null);
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.errors && result.errors.length > 0) {
          message.warning(
            `部分同步失败: ${result.errors.slice(0, 3).join(", ")}`,
          );
        }
        fetchData(); // 重新获取数据
        setSelectedRowKeys([]);
  ...
  const columns = [
    {
      ...
      render: (record: AccountStatusMismatch) => (
        <div>
          <div>
            <strong>{record.social_account.account}</strong>
          </div>
    ...
    },
    {
      ...
      render: (record: AccountStatusMismatch) => {
        const onlineStatus = getOnlineStatusText(
          record.social_account.online_status,
        );
        const accountStatus = getAccountStatusText(
          record.social_account.account_status,
        );
        return (
          <div>
      ...
      render: (record: AccountStatusMismatch) => (
        <Space>
          ...
          <Button
            ...
            loading={syncing}
            onClick={() =>
              handleSync(false, record.social_account.app_unique_id)
            }
          >
            同步
          </Button>
          ...
          <Row gutter={16} style={{ marginBottom: 16 }}>
            ...
            <Col span={6}>
              <Statistic
                ...
                valueStyle={{
                  color:
                    filteredAndSortedData.length === data.mismatchCount
                      ? "#3f8600"
                      : "#1890ff",
                }}
              />
            ...
            <Descriptions title="数据库信息" bordered size="small" column={3}>
                ...
                <Tag
                  color={
                    getAccountStatusText(
                      selectedRecord.social_account.account_status,
                    ).color
                  }
                >
                  {
              ...
              <Descriptions.Item label="在线状态">
                <Tag
                  color={
                    getOnlineStatusText(
                      selectedRecord.social_account.online_status,
                    ).color
                  }
                >
                  {
            ...
            {selectedRecord.redis_exists && (
              ...
              >
                <Descriptions.Item label="在线状态">
                  <Tag
                    color={
                      selectedRecord.redis_info.online ? "success" : "default"
                    }
                  >
                ...
                <Descriptions.Item label="心跳超时">
                  <Tag
                    color={
                      selectedRecord.is_hb_time_out ? "warning" : "success"
                    }
                  >
