...
const AccountSyncLog: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    let filtered = data.logs.filter((log) => {
      const accountMatch =
        !searchAccount ||
        log.accountInfo.account
          .toLowerCase()
          .includes(searchAccount.toLowerCase()) ||
        log.accountInfo.app_unique_id
          .toLowerCase()
          .includes(searchAccount.toLowerCase());
      ...
      const syncTypeMatch =
        syncTypeFilter === "all" || log.syncType === syncTypeFilter;
    ...
    // 按同步时间排序（最新的在前）
    filtered.sort(
      (a, b) => new Date(b.syncTime).getTime() - new Date(a.syncTime).getTime(),
    );
  ...
  const handleDownload = async () => {
    try {
      ...
      const url = window.URL.createObjectURL(
        new Blob([JSON.stringify(blob, null, 2)]),
      );
      ...
      link.setAttribute(
        "download",
        `account_sync_log_${startStr}_to_${endStr}.json`,
      );
  ...
  const getStatusChangeTag = (beforeStatus: number, afterStatus: number) => {
    ...
    return (
      <Space size={4}>
        <Tag color={getStatusColor(beforeStatus)}>
          {getStatusText(beforeStatus)}
        </Tag>
        <Text type="secondary">→</Text>
        <Tag color={getStatusColor(afterStatus)}>
          {getStatusText(afterStatus)}
        </Tag>
      </Space>
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 150,
      render: (record: AccountSyncLogEntry) =>
        getStatusChangeTag(record.beforeStatus, record.afterStatus),
    },
    {
      title: "结果",
      dataIndex: "success",
    ...
    return (
      <Card size="small" style={{ margin: "8px 0" }}>
        <Space direction="vertical" style={{ width: "100%" }}>
          ...
          <Text type="secondary">
            平台ID: {record.accountInfo.platform_id} | 账号数量:{" "}
            {record.accountsCount} | 同步时间:{" "}
    ...
    <div>
      ...
      <Card style={{ marginBottom: 16 }}>
        <Space>
          ...
          <Button icon={<CalendarOutlined />}>定期清理日志</Button>
        </Space>
      </Card>
      ...
      <Card>
        <Table
          ...
          rowKey={(record) =>
            `${record.syncTime}-${record.accountInfo.app_unique_id}-${record.syncType}`
          }
          loading={loading}
          expandable={{
            expandedRowRender,
