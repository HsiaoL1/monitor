...
const TraceAnalysis: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<"tree" | "timeline" | "flamegraph">(
    "tree",
  );
  ...
  const generateMockTrace = (traceId: string): TraceData => {
    ...
    const uniqueServices = Array.from(
      new Set(allSpans.map((s) => s.serviceName)),
    );
  ...
  const buildTreeData = (span: Span): DataNode => {
    ...
    const percentage = traceData
      ? ((span.duration / traceData.duration) * 100).toFixed(1)
      : "0";
  ...
  const renderTimelineView = () => {
    ...
    const sortedSpans = [...traceData.spans].sort(
  ...
  const renderFlamegraphView = () => {
    ...
    const renderSpanBar = (span: Span, depth: number = 0) => {
      ...
      return (
        <div
          key={span.spanId}
          style={{ marginLeft: depth * 20, marginBottom: 4 }}
        >
          <div
            style={{
              height: 24,
          ...
          >
            <Tooltip
              title={`${span.serviceName} - ${span.operationName} (${span.duration.toFixed(2)}ms)`}
            >
              <span
                style={{
                  overflow: "hidden",
  ...
  return (
    <Card
      ...
      extra={
        <Space>
          <Button onClick={generateSampleTraceId}>生成示例TraceID</Button>
          <Button
            icon={<ReloadOutlined />}
            onClick={searchTrace}
            loading={loading}
          ...
          <Row gutter={16} style={{ marginBottom: 24 }}>
            ...
            <Col span={4}>
              <Statistic
                ...
                valueStyle={{
                  color: traceData.errorCount > 0 ? "#cf1322" : "#3f8600",
                }}
          ...
          <div>
            <Descriptions title="基本信息" bordered size="small">
              ...
              <Descriptions.Item label="服务名">
                {getServiceIcon(selectedSpan.serviceName)}{" "}
