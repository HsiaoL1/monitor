...
const LogAggregation: React.FC = () => {
  ...
  const generateMockLogs = (): LogEntry[] => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      const timestamp = dayjs()
        .subtract(Math.random() * 3600, "second")
      ...
      const spanId = traceId
        ? `span-${Math.random().toString(36).substring(2, 10)}`
        : undefined;
    ...
    try {
      ...
      if (query.services.length > 0) {
        filteredLogs = filteredLogs.filter((log) =>
          query.services.includes(log.service),
        );
      }

      if (query.levels.length > 0) {
        filteredLogs = filteredLogs.filter((log) =>
          query.levels.includes(log.level),
        );
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "service",
      width: 120,
      render: (service: string) => <Tag color="geekblue">{service}</Tag>,
    },
    {
      title: "消息",
      key: "message",
      render: (record: LogEntry) => (
        <div>
          <div style={{ wordBreak: "break-word" }}>{record.message}</div>
          {record.traceId && (
    ...
    <Card
      title={
        <Space>
          <FileTextOutlined />
          日志聚合查询
          {realTimeMode && <Badge status="processing" text="实时模式" />}
        </Space>
      }
      extra={
        <Space>
              ...
              <Row gutter={16}>
                ...
                <Col span={8}>
                  ...
                  <Input
                    ...
                    value={query.traceId}
                    onChange={(e) =>
                      setQuery({ ...query, traceId: e.target.value })
                    }
                    allowClear
                  />
                </Col>
        ...
        {selectedLog && (
          <div>
            ...
            <Row gutter={16} style={{ marginTop: 8 }}>
              <Col span={12}>
                <strong>服务:</strong>{" "}
                <Tag color="geekblue">{selectedLog.service}</Tag>
              </Col>
              <Col span={12}>
                <strong>请求ID:</strong>{" "}
            ...
            {selectedLog.traceId && (
              <Row gutter={16} style={{ marginTop: 8 }}>
                ...
                <Col span={12}>
                  ...
                  <Text code>{selectedLog.spanId}</Text>
                </Col>
              </Row>
            )}
